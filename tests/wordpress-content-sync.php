...
$postState = file_get_contents($root . '/includes/Content/PostState.php');
...
$assert(is_string($acfFields) && is_string($postState) && is_string($stateToken) && is_string($snapshots), 'Request-safety support source files could not be read.');
...
$assert(str_contains($content, 'get_field_objects(') && str_contains($content, 'AcfFields::validate_post(') && str_contains($acfFields, 'update_field('), 'ACF read/write integration is missing.');
$assert(str_contains($acfFields, 'acf_get_field_groups(') && str_contains($acfFields, 'acf_get_fields(') && str_contains($acfFields, 'delete_field('), 'ACF first-write identity or authoritative rollback support is missing.');
...
$assert(str_contains($content, 'state_token(') && str_contains($content, 'assert_state_token(') && str_contains($stateToken, 'hash_hmac('), 'Opaque fresh-state tokens are missing.');
...
$assert(str_contains($manager, 'PostState::read(') && str_contains($manager, 'extra_state(') && str_contains($manager, 'PostState::apply('), 'Snapshot recovery does not include extended WordPress state.');
$assert(str_contains($manager, '$this->content->apply( $post_id, $rollback )'), 'Generic WordPress rollback is missing.');
$assert(str_contains($snapshots, '_ejb_snapshot_extra_state') && str_contains($snapshots, '_ejb_snapshot_extra_hash'), 'Durable snapshots do not integrity-protect extended state.');
...
$assert(str_contains($requests, 'ejb_content_requests_lock') && str_contains($requests, 'add_option( self::PROCESS_LOCK_OPTION') && str_contains($requests, 'compare_and_swap_process_lock('), 'Atomic request-processing lock is missing.');
$assert(!str_contains($requests, 'delete_option( self::PROCESS_LOCK_OPTION )'), 'Request lock takeover can still unconditionally delete another process lock.');
$assert(str_contains($requests, "should_process( Settings::get( 'auto_apply', 0 ) )"), 'Repository request dispatch does not honor the auto-apply opt-in.');
...
$assert(str_contains($posts, "'expected_state_token'") && str_contains($posts, 'assert_state_token('), 'Page/post requests do not reject stale target state.');
$assert(str_contains($posts, "'before_request_update'") && str_contains($posts, '$this->snapshots->create('), 'Direct post updates do not create a durable snapshot before mutation.');
...
$assert(str_contains($products, "'expected_state_token'") && str_contains($products, 'assert_state_token('), 'WooCommerce product requests do not reject stale target state.');
...
$assert(str_contains($variations, "'expected_state_token'") && str_contains($variations, 'StateToken::assert_matches('), 'Variation requests do not reject stale target state.');
...
$assert(str_contains($terms, '\\WPSEO_Taxonomy_Meta::set_values(') && str_contains($terms, 'AcfFields::validate_taxonomy('), 'Taxonomy ACF/Yoast integration is incomplete.');
$assert(str_contains($terms, "'expected_state_token'") && str_contains($terms, 'StateToken::assert_matches('), 'Taxonomy requests do not reject stale target state.');
