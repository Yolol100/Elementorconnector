...
6. Automatic remote apply is opt-in and disabled by default. Both manual and automatic apply paths must revalidate fresh GitHub/local state, refuse conflicts, create a snapshot, validate, save through Elementor, verify readback and roll back on failure.
