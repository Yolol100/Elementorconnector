...
final class AdminPage {
	...
	public function render(): void {
		...
		<div class="wrap ejb-wrap">
			...
			<p class="description"><?php echo esc_html__( 'Elementor JSON stays in WordPress and GitHub. ChatGPT/Codex can edit the GitHub copy; WordPress validates and applies it safely.', 'elementor-json-bridge' ); ?></p>
			...
			<section class="ejb-card">
				...
				<p>
					<strong><?php echo esc_html__( 'Status:', 'elementor-json-bridge' ); ?></strong>
					<?php echo $this->auth->is_connected() ? esc_html__( 'Connected', 'elementor-json-bridge' ) : esc_html__( 'Not connected', 'elementor-json-bridge' ); ?>
				</p>
			...
			<section class="ejb-card">
				<h2><?php echo esc_html__( '2. Settings', 'elementor-json-bridge' ); ?></h2>
				<form method="post" action="options.php">
					<?php settings_fields( 'ejb_settings_group' ); ?>
					<table class="form-table" role="presentation">
						<tr>
							<th scope="row"><label for="ejb-client-id"><?php echo esc_html__( 'GitHub App Client ID', 'elementor-json-bridge' ); ?></label></th>
							<td><input class="regular-text code" id="ejb-client-id" name="<?php echo esc_attr( Settings::OPTION ); ?>[github_client_id]" value="<?php echo esc_attr( (string) $settings['github_client_id'] ); ?>"><p class="description"><?php echo esc_html__( 'Public Client ID only. Do not paste a client secret or personal access token.', 'elementor-json-bridge' ); ?></p></td>
						</tr>
						<tr>
							<th scope="row"><label for="ejb-owner"><?php echo esc_html__( 'Repository owner', 'elementor-json-bridge' ); ?></label></th>
							<td><input class="regular-text" id="ejb-owner" name="<?php echo esc_attr( Settings::OPTION ); ?>[repo_owner]" value="<?php echo esc_attr( (string) $settings['repo_owner'] ); ?>"></td>
						</tr>
						<tr>
							<th scope="row"><label for="ejb-repo"><?php echo esc_html__( 'Repository name', 'elementor-json-bridge' ); ?></label></th>
							<td><input class="regular-text" id="ejb-repo" name="<?php echo esc_attr( Settings::OPTION ); ?>[repo_name]" value="<?php echo esc_attr( (string) $settings['repo_name'] ); ?>"></td>
						</tr>
						<tr>
							<th scope="row"><label for="ejb-branch"><?php echo esc_html__( 'Branch', 'elementor-json-bridge' ); ?></label></th>
							<td><input class="regular-text code" id="ejb-branch" name="<?php echo esc_attr( Settings::OPTION ); ?>[repo_branch]" value="<?php echo esc_attr( (string) $settings['repo_branch'] ); ?>"></td>
						</tr>
						<tr>
							<th scope="row"><label for="ejb-root"><?php echo esc_html__( 'JSON folder', 'elementor-json-bridge' ); ?></label></th>
							<td><input class="regular-text code" id="ejb-root" name="<?php echo esc_attr( Settings::OPTION ); ?>[repo_root]" value="<?php echo esc_attr( (string) $settings['repo_root'] ); ?>"></td>
						</tr>
						<tr>
							<th scope="row"><?php echo esc_html__( 'Automatic export', 'elementor-json-bridge' ); ?></th>
	...
	private function document_table(): void {
		if ( ! class_exists( '\\Elementor\\Plugin' ) ) {
			echo '<p>' . esc_html__( 'Activate Elementor to list documents.', 'elementor-json-bridge' ) . '</p>';
			return;
		}

		$post_types = array_values( get_post_types( [ 'show_ui' => true ], 'names' ) );
		...
		<table class="widefat striped ejb-table">
			<thead><tr><th scope="col"><?php echo esc_html__( 'Document', 'elementor-json-bridge' ); ?></th><th scope="col"><?php echo esc_html__( 'Type', 'elementor-json-bridge' ); ?></th><th scope="col"><?php echo esc_html__( 'Status', 'elementor-json-bridge' ); ?></th><th scope="col"><?php echo esc_html__( 'GitHub path', 'elementor-json-bridge' ); ?></th><th scope="col"><?php echo esc_html__( 'Actions', 'elementor-json-bridge' ); ?></th></tr></thead>
			<tbody>
			<?php foreach ( $ids as $id ) : ?>
				<?php
				$id = (int) $id;
				if ( ! $this->documents->is_elementor_document( $id ) || ! current_user_can( 'edit_post', $id ) ) {
					continue;
				}
				$post       = get_post( $id );
				...
				<tr>
					...
					<td><?php echo esc_html( $this->documents->document_type( $id ) ); ?></td>
					<td><code><?php echo esc_html( $enabled ? $status : 'disabled' ); ?></code><?php if ( $last_error ) : ?><br><span class="ejb-error"><?php echo esc_html( $last_error ); ?></span><?php endif; ?></td>
					<td><code><?php echo esc_html( $this->sync->path_for( $id ) ); ?></code></td>
					<td class="ejb-actions">
						<button type="button" class="button ejb-doc-action" data-action="toggle" data-id="<?php echo esc_attr( (string) $id ); ?>"><?php echo $enabled ? esc_html__( 'Disable', 'elementor-json-bridge' ) : esc_html__( 'Enable', 'elementor-json-bridge' ); ?></button>
						<?php if ( $enabled ) : ?>
							<button type="button" class="button ejb-doc-action" data-action="export" data-id="<?php echo esc_attr( (string) $id ); ?>"><?php echo esc_html__( 'Export', 'elementor-json-bridge' ); ?></button>
